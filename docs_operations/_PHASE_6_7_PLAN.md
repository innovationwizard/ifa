...
- [x] Zero-transaction users still see the S-2.9 empty state
      unchanged
- [x] Non-zero users see the Health Score bullet graph prominently
      above the fold
- [x] No score yet → bullet graph shows "Sube tu primer estado de
      cuenta" empty state with a "Calcular ahora" button
- [x] Monthly summary respects the current-month boundary in
      `America/Guatemala`
- [x] Recent-activity rows match the feed's row style for
      consistency
- [x] Full gate sweep green
...
- [x] Endpoint requires `CRON_SECRET`; 401 otherwise
- [x] Per-profile failures don't crash the batch; failed profile
      ids logged with the error
- [x] Each profile's recompute respects the 1× / hour throttle
      from Batch 11 (cron bypasses it via a `force: true` flag)
- [x] Summary returned: `{ totalProfiles, succeeded, failed,
durationMs }`
- [x] Vercel cron config registers the schedule
- [x] Full gate sweep green
...
- [x] Batch 14 — Dashboard wire-up · 6/6 in batch · 95/101 overall · commit `<pending>` · 2026-05-21 - `src/lib/reports/current-month.ts` exports `currentMonthInGuatemala(now?)`: derives the calendar month via `Intl.DateTimeFormat` with `timeZone: 'America/Guatemala'`, then constructs UTC midnight bounds for `[from, to]`. Most of the codebase pretends UTC=GT for month bucketing (correct except in the 6h window before midnight UTC at month edges); the dashboard is the one place that matters, so this helper does it right. Defensive fallback to UTC bounds if `Intl` ever fails - `src/components/dashboard/score-widget.tsx` server component wraps `<HealthScoreBullet>` (Batch 12) in a card with a "Ver detalle" link to `/dashboard/salud`. No-score-yet branch renders a Sparkles icon + "Calcular ahora" CTA linking to the detail page (the detail page owns the `recomputeNow` server action — single source of truth for the throttle check) - `src/components/dashboard/monthly-summary.tsx` server component: three-up grid (ingresos / gastos / neto) using `<Money>`, color-toned (green positive, red negative, navy neutral). Defensive fallback to zero-bucket when `monthlyCashFlow` returns empty - `src/components/dashboard/recent-activity.tsx` server component: last 10 transactions, server-rendered (no virtualization — fixed cap doesn't warrant it). Row layout mirrors the feed's column order (date / description / merchant / amount / source) so the dashboard feels like a trimmed version of `/transacciones`. No badges (duplicate/anomaly) — the score widget surfaces those holistically; repeating them per-row would compete with the score itself - `src/app/(app)/dashboard/page.tsx`: branch unchanged for zero-tx (S-2.9 empty state), non-zero renders three-zone layout: header greeting → 2-col on desktop (score + recent left, monthly + quick-links right), vertical stack on mobile. Quick-links card mirrors the `/reportes` hub: BarChart3/PieChart/Store linking to flujo / gastos / comercios. Single `withTenant` wraps `transactionRepo.count` then `Promise.all` over the three independent reads (latest score, current-month rows, recent-10) - `src/messages/es-GT.json` `dashboard.*` extended: `home` (greeting + subtitle), `score`, `monthlySummary`, `recentActivity`, `quickLinks` — all in tú-register lower-elementary Spanish - Unit tests: 6 in `current-month.test.ts` (mid-month happy path, 02:00 UTC before-midnight GT edge → previous month, 06:00 UTC = midnight GT rollover, Dec→Jan year rollover, 28-day February non-leap, 29-day February leap year) - No new e2e: `/dashboard` auth redirect already covered by `tests/e2e/auth-redirects.spec.ts` - gate sweep: lint ✓ typecheck ✓ prettier ✓ vitest 530/530 ✓ playwright 35/35 chromium ✓ next build ✓
- [x] Batch 15 — Nightly cron stub · 6/6 in batch · 101/101 overall · commit `<pending>` · 2026-05-21 - `profileRepo.listActiveProfileIds()` returns `id[]` for every profile whose `subscriptionStatus !== 'EXPIRED'` (TRIAL / ACTIVE / EARLY_SUPPORTER / PAST_DUE / CANCELED — everyone with UI access, even during grace periods). Selects ids only so the cron's memory footprint stays flat regardless of tenant count; ordered by `createdAt` so the per-profile isolation log is deterministic - `src/lib/intelligence/health-score/cron-runner.ts` exports `runHealthScoreCron({ now? }) => CronSummary`. Loops profile ids, calls `recomputeHealthScore({ profileId, now, period: 'DAILY' })`, then stamps `Profile.lastHealthScoreRecomputeAt` so a user who logs in within the next hour and presses "Recalcular" gets the 429 (their score is already fresh from the cron). Per-profile try/catch isolates failures — one throwing recompute does not crash the batch; failed profile ids land in the `failures: []` array with the serialized error message AND are logged via `console.error` for Vercel-logs visibility. Header comment documents the Railway migration path (when this hits the 60s Vercel ceiling: Railway worker `curl`s this same endpoint with `Authorization: Bearer $CRON_SECRET`; no code changes needed) - `src/app/api/cron/health-score/route.ts` GET handler: Bearer-token auth via `CRON_SECRET` (fail-closed when env unset, same shape as `/api/cron/jobs` from Batch 4), delegates to `runHealthScoreCron()`, returns the summary as JSON - `vercel.json` registers `0 8 * * *` (08:00 UTC = 02:00 GT) cron pointing at `/api/cron/health-score`. Vercel cron auto-attaches the `Authorization: Bearer $CRON_SECRET` header - Unit tests: 7 in `cron-runner.test.ts` (empty profile set, happy path verifying period: 'DAILY' + per-profile stamp, per-profile isolation when recompute throws — verifies failed profile is NOT stamped so a retry sees stale lastHealthScoreRecomputeAt, isolation when the stamp itself throws after a successful recompute, non-Error throw serialization) + 4 in `route.test.ts` (CRON_SECRET unset, no Auth header, wrong bearer, happy path with summary JSON shape) = 11 new. E2E: 3 auth specs × 3 browsers = 9 in `tests/e2e/api-cron-health-score.spec.ts` - Risk note from the plan still applies (≲500 profiles at ~100ms each fits the 60s Vercel ceiling; migrate to Railway past that threshold — the endpoint shape is already what an external scheduler expects) - gate sweep: lint ✓ typecheck ✓ prettier ✓ vitest 539/539 ✓ playwright 38/38 chromium ✓ next build ✓
