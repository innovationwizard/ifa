...
- **Active sub-batch:** L3.5.6 — DONE locally (disconnect Google with bank-grade re-auth gate)
- **Last commit relevant to Phase L:** `70a2da9` (L3.5.5 connect Google with re-auth gate)
- **Next concrete action:** Founder commits L3.5.6. After push, L3.5 group is complete (.1 + .5 + .6). Doc advances to L3.6 (data export) — the "Tus datos" section currently renders the `placeholderSoon` copy. L3.6 needs: a server action that gathers all tenant-scoped data (profile, transactions, categorizations, health scores), zips it, returns a signed URL or streams directly. Friends-and-family beta scope: synchronous in-request build (data volumes are tiny); promote to job queue if it ever exceeds 60s. Refactor candidate noted in actions.ts: confirmEmailChange / confirmGoogleLink / confirmGoogleUnlink share the (auth + freshness + pending-metadata) gate shape — extract a shared helper at L3 closure, not now (would touch tested ADR-003 code paths).
- **Blockers:** none.
- **Files in flight (uncommitted edits):** `src/app/(app)/configuracion/actions.ts` (added `requestGoogleUnlink` + `confirmGoogleUnlink` — same 3-factor gate as L3.5.5 plus 2 extra gates: Google still linked + ≥2 identities total) + `src/app/(app)/configuracion/confirmar-desconectar-google/page.tsx` (NEW — gate-checking page; `not_linked` + `last_identity` blocked states added) + `src/components/settings/account-card.tsx` (Desconectar form + last-identity hint copy + unlinkError/unlinkedJustNow props + identityCount prop) + `src/app/(app)/configuracion/page.tsx` (derives googleLinked/identityCount once, threads `?unlinked=google` / `?unlinkError=`) + `src/messages/es-GT.json` (added `signInMethods.google.disconnect.*` + `.disconnectPage.*` + `unlinkedJustNow`; dropped now-unused `disconnectSoon`) + `src/app/(app)/configuracion/actions.test.ts` (added 8 unit tests for confirmGoogleUnlink gates incl. last_identity + not_linked + happy-path unlink — total file now covers 15 gate cases across .5 + .6).
