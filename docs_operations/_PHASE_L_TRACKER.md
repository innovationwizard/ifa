...
- **Active batch:** L6 — Pre-launch polish (privacy, terms, support)
- **Active sub-batch:** L6 — DONE locally (full USTED-register /privacidad + /terminos rewrites, NEW /contacto, wizard failure-alert link, /ingresar support link, launch QA checklist)
- **Last commit relevant to Phase L:** `0f6d4bb` (L5 Stripe live wiring)
- **Next concrete action:** Founder commits L6. After push, the only remaining batch is L7 — Launch checklist (Vercel custom domain, DNS, monitoring/Sentry decision, error pages audit, op runbook). Before L7 starts, founder should walk through `docs_operations/_LAUNCH_QA_CHECKLIST.md` against the production deploy — that's the manual gate the L6 plan acceptance item requires, and any blockers found become L7 carry-over.
- **Files in flight (uncommitted edits):** `src/app/contacto/page.tsx` (NEW USTED-register support page — founder email + 1-2-day response copy + cross-links) + `src/app/privacidad/page.tsx` (full USTED rewrite — summary + 5 data categories + 6 processors enumerated (Supabase, Vercel, Stripe, Resend/AWS SES, Anthropic, Google) + user rights + 30-day retention + security posture + changes + contact) + `src/app/terminos/page.tsx` (full USTED rewrite — 15 sections: acceptance, service scope, account, pricing $1/$20 USD with FX caveat, payment+renewal, cancellation, refunds, deletion+30-day grace, export, acceptable use, IP, liability cap, GT governing law, changes, contact) + `src/messages/es-GT.json` (replaced placeholder `legal.{terms,privacy}` blocks with full USTED copy; added `legal.contact.*` block; added `auth.signIn.supportPrefix/supportLink` + `imports.result.failureSupportLink`) + `src/app/ingresar/page.tsx` (added support link below the terms/privacy footer line) + `src/components/imports/csv-import-wizard.tsx` (failure alert includes a link to /contacto so users with broken imports have a clear next step) + `docs_operations/_LAUNCH_QA_CHECKLIST.md` (NEW — 9-flow code-derived QA walkthrough covering pre-flight, sign-in, onboarding, dashboard, import (incl. AI fallback paths), transactions, reports, every L3 settings sub-flow incl. all bank-grade confirm pages, legal pages, hard-gate paywall via SQL fast-forward; per-flow known-gotchas list + "Blockers found" log at the bottom).
