...
- **Active batch:** L7 — Launch checklist
- **Active sub-batch:** L7 — DONE locally (sitemap + robots + launch ops checklist). **PHASE L CODE COMPLETE — every batch L1–L7 is implemented.**
- **Last commit relevant to Phase L:** `536be51` (L6 legal pages + QA checklist)
- **Next concrete action:** Founder commits L7. After push, the remaining work is purely operational: (1) walk through `docs_operations/_LAUNCH_QA_CHECKLIST.md` against the production deploy to verify every app flow works; (2) walk through `docs_operations/_LAUNCH_CHECKLIST.md` to verify every ops dependency is configured (Stripe live, email DNS, Supabase backups, domain choice, beta-invite mechanism); (3) when both checklists are clean, run the §9 go/no-go ceremony and send the first beta invite. No more code remains to ship for the friends-and-family beta launch. Post-launch debt (re-auth-gate refactor, hard-delete cleanup job, L2 sample collection, app-level email callsites) lives at the bottom of `_LAUNCH_CHECKLIST.md`.
- **Files in flight (uncommitted edits):** `src/app/sitemap.ts` (NEW — Next 13+ sitemap convention; lists the 6 public URLs with weekly/monthly changeFrequency; `lastModified` omitted on purpose so crawlers don't get a misleading "everything changed" signal on every deploy) + `src/app/robots.ts` (NEW — explicit Allow for the 6 public routes + Disallow for every authed surface and `/api/` + `/auth/`; defense in depth on top of proxy redirects; references the sitemap URL) + `docs_operations/_LAUNCH_CHECKLIST.md` (NEW — 9-section ops verification: Stripe live $1 charge end-to-end, email DNS verification, Supabase backups + RLS + manual-linking toggle, Vercel deploy posture + no-crons per ADR-002, domain decision, SEO check, beta-invite mechanism, 48-hour observability watchlist, go/no-go ceremony with first-invite slot, post-launch 30-day metrics list).

## §1.5 — Phase L milestone (added 2026-06-02)

**Phase L code is complete.** L1 universal AI ingestion + L2 PDF + L3 settings (9 sub-batches) + L4 email foundation + L5 Stripe live + L6 legal/QA + L7 launch checklist. All shipped locally; L7 is the last code commit. After founder's commit + push, the remaining work is purely the two checklist doc walkthroughs and the go/no-go ceremony. No further `pnpm` commands beyond what L7's commit message lists.
