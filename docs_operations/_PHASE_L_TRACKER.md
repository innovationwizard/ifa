...
- **Active sub-batch:** L3.6 — DONE locally (data export ZIP download)
- **Last commit relevant to Phase L:** `9bd8cfb` (L3.5.6 disconnect Google)
- **Next concrete action:** Founder commits L3.6. After push, doc advances to L3.7 (account deletion) — type-to-confirm pattern + softDelete cascade. Critical: account deletion must (1) softDelete Profile+ProfileMember rows, (2) revoke Stripe subscription if active, (3) keep tenant audit logs (compliance), (4) sign the user out, (5) display a confirmation page they can't accidentally trigger from a misclick (require typing "ELIMINAR MI CUENTA" verbatim per ADR-003 bank-grade posture). Also queued: the i18n namespace fix noted in this commit (sections.X.Y was the JSON location, components were reading X.Y for L3.3–L3.5.6 — silently broken until L3.6 caught it).
- **Blockers:** none.
- **Files in flight (uncommitted edits):** `src/lib/export/build-zip.ts` (NEW pure function — gathers Profile/User/ProfileMember/Transactions/HealthScores via repos, fetches uploaded originals from Supabase storage `imports` bucket, returns ZIP bytes via jszip) + `src/lib/export/build-zip.test.ts` (NEW — 8 unit tests covering empty/populated states, JSON shape, CSV RFC-4180 quoting + BOM, originals fetch, per-file error tolerance, README counts) + `src/app/api/v1/me/export/route.ts` (NEW auth-gated GET handler — returns application/zip with Content-Disposition: attachment) + `src/components/settings/data-card.tsx` (NEW client — fetch → blob → anchor download, spinner + error states) + `src/app/(app)/configuracion/page.tsx` (renders DataCard for the `data` section) + `src/messages/es-GT.json` (added `settings.sections.data.{body|cta|preparing|error.*}`) + i18n namespace fix in `profile-card.tsx` / `account-card.tsx` / `data-card.tsx` + 3 confirm pages (`settings.X.*` → `settings.sections.X.*` — pre-existing bug from L3.3 caught while wiring L3.6, components were silently rendering key paths instead of strings) + 3 repo files (added `transactionRepo.listAllForExport`, `healthScoreRepo.listAllForExport`, `profileRepo.listMembersForExport` — direct prisma access is restricted outside src/lib/db, so export goes through repos) + `package.json` (added jszip dependency).
