...
- **Active batch:** L4 — Transactional email (Resend OR AWS SES)
- **Active sub-batch:** L4 foundation — DONE locally (provider-agnostic layer + Supabase SMTP relay docs)
- **Last commit relevant to Phase L:** `9052589` (L3.7 account deletion)
- **Next concrete action:** Founder commits L4. After push, the founder configures EITHER Resend or AWS SES at the provider level (DKIM/SPF DNS + API key/SES creds) AND at the Supabase dashboard (Auth → Emails → SMTP Settings) per the new §2.5 in vercel-setup.md. Once that's done, Supabase auth emails (magic links, email-change OTPs, delete OTPs) start coming from the IFA-branded sender via the chosen provider. The codebase has zero app-level email callsites today (founder chose "foundation only"); the first send callsite will land with L5 (billing-receipt on Stripe checkout success) or sooner if we want a welcome/deletion-receipt email. **Doc advances to L5 — Stripe live wiring.**
- **Blockers:** none. L4 codebase changes are foundation-only; runtime email delivery requires the founder's Vercel env-var + Supabase SMTP config.
- **Files in flight (uncommitted edits):** `src/lib/email/` (NEW directory — types.ts with EmailMessage + EmailProvider interface + formatAddress RFC 5322 helper + EmailSendError; providers/resend.ts + providers/ses.ts using the official `resend` SDK + `@aws-sdk/client-sesv2`; client.ts with env-driven provider selector + module-level cache + test-only reset; send.ts with retry wrapper + skipped-when-no-provider behavior + recipient log preview; index.ts barrel; email.test.ts with 10 unit tests covering address quoting, provider routing, skipped-state, default-from fallback, missing-from error, retry success on later attempt, retry exhaustion) + `src/lib/env.ts` (added `getEmailEnv()` mirroring the Stripe pattern — returns null when EMAIL*PROVIDER unset) + `.env.example` (added EMAIL_PROVIDER + EMAIL_FROM*_ + RESEND*API_KEY + AWS_SES*_ with provider-choice docstring) + `docs_operations/vercel-setup.md` (NEW §2.5 with provider comparison table, Resend setup walkthrough, SES setup walkthrough, Supabase Custom SMTP relay configuration for BOTH providers, verify/rotate guidance) + `package.json`/`pnpm-lock.yaml` (resend ^6.12.4 + @aws-sdk/client-sesv2 ^3.1059.0).
