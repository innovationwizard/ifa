...
- **Active batch:** L3 — Settings (account hygiene) — ALL CORE SUB-BATCHES DONE
- **Active sub-batch:** L3.7 — DONE locally (account deletion: soft-delete + Supabase ban + type-to-confirm "ELIMINAR MI CUENTA")
- **Last commit relevant to Phase L:** `33617ed` (L3.6 data export)
- **Next concrete action:** Founder commits L3.7. After push, the L3 (Settings) batch is feature-complete for INDIVIDUAL MVP: L3.1 schema + L3.2 shell + L3.3 profile + L3.4 email change + L3.5 sign-in methods read + L3.5.5 connect Google + L3.5.6 disconnect Google + L3.6 data export + L3.7 account deletion. **Doc advances to L4 — Transactional email (Resend).** L4 wires Supabase Auth's SMTP / custom email-sender so every magic link and email change/delete confirmation goes out reliably (currently relies on Supabase's default sender, which has lower deliverability + Supabase branding). Also queued post-L3 polish: (a) extract the shared (auth + freshness + pending-metadata) re-auth gate helper now repeated 4× in actions.ts (deferred from L3.5.5, .5.6, .7); (b) hard-delete cleanup job that purges soft-deleted profiles after a 30-day grace period.
- **Blockers:** none.
- **Files in flight (uncommitted edits):** `src/lib/db/repositories/profile.ts` (added `softDeleteAccount` atomic transaction + filtered `findManyForUser` to exclude soft-deleted rows) + `src/app/(app)/configuracion/actions.ts` (added `requestAccountDeletion` + `confirmAccountDeletion` with 4 gates incl. type-to-confirm phrase check; soft-deletes via repo, bans Supabase Auth via admin API ban_duration: '876000h', signs out) + `src/app/(app)/configuracion/confirmar-eliminar-cuenta/page.tsx` (NEW gate-checking page) + `src/components/settings/delete-confirm-form.tsx` (NEW small client form for live type-to-confirm UX + redirect to /ingresar?deleted=1 on success) + `src/components/settings/delete-card.tsx` (NEW kick-off card on /configuracion with destructive warning + Empezar button) + `src/app/(app)/configuracion/page.tsx` (replaced placeholder with `<DeleteCard />`) + `src/app/ingresar/page.tsx` (adds goodbye banner when `?deleted=1`) + `src/messages/es-GT.json` (added `settings.sections.delete.{warningTitle|warningBody|cta|sending|linkSentTitle|linkSentBody|error.*|confirmPage.*}` + `auth.signIn.deletedTitle/deletedBody`; dropped now-unused `settings.placeholderSoon`) + `src/app/(app)/configuracion/actions.test.ts` (added 11 unit tests for confirmAccountDeletion covering every gate state incl. phrase-mismatch case-sensitivity + whitespace tolerance + ban-after-DB-succeeded recovery semantics).
