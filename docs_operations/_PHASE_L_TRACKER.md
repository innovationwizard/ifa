...
- **Active sub-batch:** L1.2.5 — DONE locally, awaiting founder commit + push
- **Last commit relevant to Phase L:** `39d9f94` (L1.2 heuristic wrapper)
- **Next concrete action:** Founder commits L1.2.5 (`src/lib/ingestion/projection.ts` + refactored `heuristic-detect.ts`). After push, this doc marks L1.2.5 closed and advances to L1.3 (`ai-detect.ts` — Claude Haiku extractor using the now-shared `projectCsvSample`).
- **Blockers:** §6.1 RESOLVED — founder collecting samples in parallel. L1.3/L1.4 unblocked.
- **Files in flight (uncommitted edits):** `src/lib/ingestion/projection.ts` (new) + `src/lib/ingestion/heuristic-detect.ts` (modified to import the shared helper)
...
- [ ] **L1.2.5** — `src/lib/ingestion/projection.ts`: extract the shared `projectCsvSample(rows, mapping) → ExtractedRow[]` helper so L1.3's ai-detect doesn't duplicate it. Refactor heuristic-detect to import from the shared module. _(Inserted between L1.2 and L1.3 when L1.3 surfaced the duplication.)_
