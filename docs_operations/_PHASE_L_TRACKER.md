...
- **Active batch:** L5 — Stripe live wiring
- **Active sub-batch:** L5 — DONE locally (StripeEventLog idempotency + webhook refactor + /configuracion/facturacion + ops docs)
- **Last commit relevant to Phase L:** `cf34332` (L4 transactional email foundation)
- **Next concrete action:** Founder runs `pnpm db:push` to land the `stripe_event_logs` table in prod Supabase (the only schema change). Then commits L5. After push, founder works through vercel-setup.md §2.6: create live products + prices in Stripe dashboard (Individual $1/mo, Business $20/mo), enable invoice emails, configure webhook endpoint, copy live `sk_live_*` + `whsec_*` + price ids into Vercel env. Validate end-to-end with a real $1 charge on the founder's card. **Doc advances to L6 — Pre-launch polish (privacy, terms, support page).**
- **Blockers:** none. Live-key cutover is founder ops; can wait until L7 launch checklist if testing happens in test-mode first.
- **Files in flight (uncommitted edits):** `prisma/schema.prisma` (NEW StripeEventLog model: id PK as Stripe event.id, eventType mirror, processedAt timestamp; indexed on processedAt + eventType+processedAt) + `src/lib/db/stripe-event-log.ts` (NEW helper — recordStripeEventOnce wraps Prisma $transaction with ON CONFLICT P2002 → 'duplicate' semantics; lives in src/lib/db/ so it can touch prisma directly under the no-restricted-imports rule) + `src/app/api/stripe/webhook/route.ts` (refactored — every event runs inside recordStripeEventOnce; added invoice.payment_succeeded ack-only handler; handlers take Prisma.TransactionClient so writes commit atomically with the event-log insert; returns `duplicate:true` body when Stripe retries) + `src/app/api/stripe/webhook/route.test.ts` (NEW — 13 unit tests covering missing-signature, invalid-signature, idempotency log insertion, duplicate response, 500-on-non-P2002, per-event mutations for checkout.session.completed, customer.subscription.{updated,deleted}, invoice.payment_failed lookup-then-PAST_DUE, invoice.payment_succeeded ack-without-mutation) + `src/app/(app)/configuracion/facturacion/page.tsx` (NEW server page — pulls gate state via computeGateState, formats date via next-intl, hands off to ManageBillingButtons) + `src/components/settings/billing-card.tsx` (NEW server component for /configuracion summary — status line + link to facturacion sub-page) + `src/components/settings/manage-billing-buttons.tsx` (NEW client — Suscribirme / Gestionar pago buttons POSTing to existing /api/stripe/{checkout,portal}; handles 401/503/error) + `src/app/(app)/configuracion/page.tsx` (added billing section between account and data; computes gateState once; renders BillingCard) + `src/messages/es-GT.json` (added settings.sections.billing.\* — 8 status reasons × 2 contexts with pluralized day countdowns) + `docs_operations/vercel-setup.md` (NEW §2.6 walking through Stripe live setup: products + prices, invoice email toggle, webhook endpoint with the 6 events to subscribe, env-var mapping, verification, rotation).
