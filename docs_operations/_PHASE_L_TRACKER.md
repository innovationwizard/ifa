...
- **Active sub-batch:** L1.6.5 — DONE locally (heuristic tests, 9/9), awaiting founder commit + push
- **Last commit relevant to Phase L:** `b9e26fa` (L1.6 orchestrator tests)
- **Next concrete action:** Founder commits L1.6.5 (`src/lib/ingestion/heuristic-detect.test.ts`, 9 passing tests). After push, this doc advances to L1.7 (`src/app/api/v1/imports/parse/route.ts` — server endpoint accepting a sampled CSV + returning ExtractorResult, called by the wizard for both heuristic-confident and AI-fallback paths).
- **Blockers:** §6.1 still open. L1.7 unblocked (route doesn't need real bank samples).
- **Files in flight (uncommitted edits):** `src/lib/ingestion/heuristic-detect.test.ts` (new, ready to commit)
