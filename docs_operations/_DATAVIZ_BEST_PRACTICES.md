...
| B13            | Factor radar chart                      | **Decision 2026-05-21: swap to horizontal factor bars.** Radar charts are critiqued by NN/g + Few alongside pies/gauges (circular charts are poor for quantitative comparison). Horizontal bars sorted descending by score, tier-colored from Batch 12's `TIER_BANDS`. Reuses B7's bar-chart pattern; same Recharts dep. |
