...
> **Version:** 1.1.0 (audience pivot 2026-04-21)
...
> **Date:** 2026-04-20 (rev. 2026-04-21)

---

## Genesis prompt (north star — never remove this block)

The scaffolding was commissioned against this prompt. Any future revision of this
document must be checked against it. If something in the rest of the document
drifts from this intent, the prompt wins.

> **"Make a comprehensive and unabridged scaffolding, from the point of view of
...
1. **No credit bureau.** Consumer credit bureaus do not exist in Guatemala in
   ...
   else. The Financial Health Score (§8) is a **proprietary score IFA builds
   from the user's own bank + card data**. This is the product's entire
   wedge and moat; it cannot be outsourced. Any onboarding, copy, or design
   that implies a bureau pull is wrong.
2. **Tiered product from day one:** `INDIVIDUAL` is the default tenant type
   (this is the Credit Karma analog and what every new user lands in).
   `BUSINESS` exists from day one as a **proactive opt-in upgrade** — the
   MIPYME features described in the business-case PDF
   (`0_Inteligencia-Financiera-App-Guatemala.pdf`) live behind this tier
   (NIT, fiscal regime, FEL issuance, TPV ingestion, chart-of-accounts,
...
- **Default ingestion (individual tier):** bank account statements +
  credit-card statements. Optional: FEL receipts the user received **as a
  consumer** (for the tax-advanced opt-in). TPV feeds activate only when a
  user upgrades to the BUSINESS tier.
- **Tax / fiscal features:** always OPTIONAL even inside BUSINESS. Taxes
  carry high emotional and friction load; don't block adoption on them.
- **Identity:** users are identified internally by **UUIDv7** generated by
  IFA. DPI (Documento Personal de Identificación) is optional metadata only
  — a photo upload or typed number, stored but not validated and never used
  as a lookup key. NIT is similarly optional, revealed in advanced mode.
- **Paywall:** deferred for MVP. Tier scaffolding is structural so pricing
  can be configured later; MVP has no billing enforcement.
- **UX vocabulary target:** lower than elementary-school Spanish, `tú`
  register (not `usted`). Avoid technical accounting and tax terms
  (reconciliación, asiento contable, NIIF-PYME, IVA crédito) in default
  flows — those live in an opt-in advanced mode.

The rest of this document (modules, data model, gamification, architecture)
still applies but must be re-read through the individual-human lens. Sections
that explicitly assume an MIPYME context (multi-entity, Canal Contable, NIT
required, TPV-primary, fiscal regime mandatory) are flagged for revision in
the accompanying build plan.
